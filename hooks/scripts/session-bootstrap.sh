...
additional_context='cursor-harness is installed. Inventory: .cursor/HARNESS.md (/help, /glossary). Domain overlay: .cursor/HARNESS.local.md when present. Project interface: harness.project.yaml. Prep (anytime, ~2h max): /prep packet grill → plan review → approve contract in a human-created Cursor worktree. Nightshift: Cursor Build or night-shift fire → execute-approved-plan (park BLOCKED.md, never wait). After: /ship-local (worktree) or leftover-commit on default, then /ship-prod. Leftover farm reset: /clean-worktrees. Task agents may create one tree via .cursor/skills/ship-local/worktree-new.sh <slug>; /ship-local removes it. Task agents never open pull requests. Never remote push/deploy without an explicit human request (/ship-prod). Chat last line DONE or PARTIAL.'
