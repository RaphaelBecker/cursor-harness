...
| Merge, remote push, production | Human (`/ship-local`, `/ship-prod`). Task agents never open pull requests. The shell guard blocks raw `git push` and `gh pr create` / `gh pr merge` unless `/ship-prod` preflight armed a fresh push gate. That marker also allows `git push origin main` inside the consumer's `vendor/cursor-harness` when the push fast-forwards `main` |
