...
| `guard-destructive-shell` | `beforeShellExecution` | Confirm destructive DB, force-push to main/master, `gh issue edit` / close / delete. Deny `find` / `du` / `rg` / `grep -r` / `ls -R` / `tree` / `fd` rooted at `/`, `/Users`, `/Volumes`, `/System/Volumes`, `/home`, `/net`: on macOS they walk the `/home` automount and pop a network-volume prompt. Test: `bash hooks/scripts/guard-destructive-shell.test.sh` | [guard-destructive-shell.sh](../hooks/scripts/guard-destructive-shell.sh) |
