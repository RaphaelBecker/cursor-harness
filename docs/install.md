...
| `night-shift fire` skips trees | Need **exactly one** `.cursor/plans/*.md` with `status: approved` **for this item** in that worktree (foreign leftovers must not be executed) |
| New worktree inherits an approved plan | Ship archives plans (`status: archived`). Create-time clears night-shift working files and does not seed `contract.md`. |
